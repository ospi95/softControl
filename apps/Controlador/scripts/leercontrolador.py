...
class Leercontrolador:
    ...
    def leer(self, port, vel, id1, id2):
        ...
        result = client.read_holding_registers(address=0x53, count=1, unit=id1)
        d = int(math.pow(10, result[0]))

        #Lectura del controlador (SV, OUT)
        result = client.read_holding_registers(address=0x02, count=2, unit=id1)
        sv1 = result[0]
        out1 = result[1]
        if sv1 > 63000:
            sv1 = sv1 - 65536
        ...
        result = client.read_holding_registers(address=0x19, count=3, unit=id1)
        p1 = result[0]
        p1 = p1/10
        i1 = result[1]
        d1 = result[2]
        ...
        result = client.read_holding_registers(address=0x100, count=1, unit=id1)
        pv1 = result[0]
        if pv1 > 63000:
            pv1 = pv1 - 65536
        pv1 = pv1/d
        ...
        if id2 != 999:
            # Lectura del controlador (Numero decimales para PV y SV)
            result = client.read_holding_registers(address=0x53, count=1, unit=id2)
            d = int(math.pow(10, result[0]))

            # Lectura del controlador (SV, OUT)
            result = client.read_holding_registers(address=0x02, count=2, unit=id2)
            sv2 = result[0]
            out2 = result[1]
            if sv2 > 63000:
                sv2 = sv2 - 65536
            ...
            # Lectura del controlador (Banda proporcional (P), tiempo integral (I) y tiempo derivativo (D))
            result = client.read_holding_registers(address=0x19, count=3, unit=id2)
            p2 = result[0]
            p2 = p2 / 10
            i2 = result[1]
            d2 = result[2]
            ...
            # Lectura del controlador (PV)
            result = client.read_holding_registers(address=0x100, count=1, unit=id2)
            pv2 = result[0]
            if pv2 > 63000:
                pv2 = pv2 - 65536
            pv2 = pv2 / d
